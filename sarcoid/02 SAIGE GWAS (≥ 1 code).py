...
if not sarcoid_1_rows.empty:
    print(f"Found {len(sarcoid_1_rows)} people with sarcoid_n==1. If running on sarcoid_n ≥ 2, will need to remove from sarcoid_without_exclusions.")
else:
...
filtered_sarcoid_df.groupby('case', as_index = False).agg({'person_id' : 'nunique'})
...
get_ipython().system('gsutil cat {bucket or my_bucket}/dsub/logs/step1-eur-run-saige-null-model/bwaxse/20250630/step1-eur---bwaxse--250630-173928-69-task-None.log')
...
get_ipython().system('gsutil ls {my_bucket}/saige_gwas/min_1/')
...
def run_saige_step2(
    ...
    for anc in ancestries:
        ...
        for chrom in chroms:
            ...
            in_dict = {
                ...
                'INPUT_NULL_RDA': f'{my_bucket}/saige_gwas/min_1/{anc}/{trait}/saige_null_model.rda',
                'INPUT_NULL_VARRAT': f'{my_bucket}/saige_gwas/min_1/{anc}/{trait}/saige_null_model.varianceRatio.txt',
            }
            ...
            dsub_script(
                label=f'step2_{anc}_chr{chrom}_{trait}',
    ...
    print(f"Submitted SAIGE step2 jobs")

def run_saige_step2_whole_cohort(
    trait,
    script,
    chroms=range(1, 23),
    ...
    """
    Run SAIGE step2 for trans-ancestry whole cohort analysis.
    
    Args:
        trait: Trait name (e.g., 'condition__hpv')
        script: Path to the step2 script
        chroms: Chromosomes to process
        machine_type: VM machine type
        disk_size: Disk size in GB
    ...
    print(f"Submitting whole cohort jobs for trait: {trait}")
    
    for chrom in chroms:
        plink_base = f'{src_bucket}/data/stg009/whole_cohort/genotypes_chr{chrom}'
        ...
        in_dict = {
            ...
            'INPUT_BIM': f'{plink_base}.bim',
            'INPUT_FAM': f'{plink_base}.fam',
            'INPUT_NULL_RDA': f'{my_bucket}/saige_gwas/transancestry/{trait}/saige_null_model.rda',
            'INPUT_NULL_VARRAT': f'{my_bucket}/saige_gwas/transancestry/{trait}/saige_null_model.varianceRatio.txt',
        }
        
        out_dict = {
            'OUTPUT_FILE': f'{output_folder}/transancestry/{trait}/gwas/gwas_results_chr{chrom}.txt',
            'OUTPUT_FILE_INDEX': f'{output_folder}/transancestry/{trait}/gwas/gwas_results_chr{chrom}.txt.index'
        }
        
        dsub_script(
            label=f'step2_chr{chrom}_{trait}',
            machine_type=machine_type,
            envs=env_dict,
            in_params=in_dict,
            out_params=out_dict,
    ...
    print(f"Submitted whole cohort SAIGE step2 jobs")
# In[ ]:


run_saige_step2(['eur'], 'condition__sarcoid', 'run_saige_chrom_multi.sh', chroms=[1,2,6])


# In[ ]:


run_saige_step2(['afr'], 'condition__sarcoid', 'run_saige_chrom_multi.sh', chroms=[1,2,6])

run_saige_step2_whole_cohort('condition__sarcoid', 'run_saige_chrom_multi.sh', machine_type='c4-standard-8')
# In[ ]:
...
job_id = 'step2-eur---bwaxse--250915-184933-66'
...
log_path = '{bucket or my_bucket}/dsub/logs/step2-eur-chr6-condition--sarcoid-run-saige-chrom-multi/bwaxse/20250915/step2-eur---bwaxse--250915-184933-66-task-None.log'
view_dsub_logs(log_path)


# In[ ]:


chr_df = pl.read_csv('{bucket or my_bucket}/saige_gwas/min_1/eur/condition__sarcoid/gwas/gwas_results_chr6.txt',
                    separator='\t')
