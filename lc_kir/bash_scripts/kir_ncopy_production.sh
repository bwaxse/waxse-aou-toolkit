...
# ============================================================================
# KIR Copy Number Production Script
# ============================================================================
# Runs GATK PrintReads → kir-mapper map → kir-mapper ncopy on production samples
# Uses pre-calibrated ancestry-specific thresholds.txt files
#
# Environment variables (set by dsub):
#   - NUM_SAMPLES: Number of samples to process (typically 200)
...
#   - INPUT_THRESHOLDS: Path to pre-calibrated thresholds.txt
...
if [ -z "$INPUT_THRESHOLDS" ]; then
    echo "ERROR: INPUT_THRESHOLDS not set (must point to pre-calibrated thresholds.txt)"
    exit 1
fi
...
echo "============================================================================"
echo "KIR Copy Number Production"
echo "============================================================================"
...
echo "Thresholds: $INPUT_THRESHOLDS"
echo "============================================================================"
...
# Step 3: kir-mapper ncopy with pre-calibrated thresholds
echo ""
echo "=== Step 3: kir-mapper ncopy (using pre-calibrated thresholds) ==="
NCOPY_START=$(date +%s)

# Copy thresholds to kir_output directory
if [ -f "$INPUT_THRESHOLDS" ]; then
    echo "Using thresholds from: $INPUT_THRESHOLDS"
    cp "$INPUT_THRESHOLDS" ./kir_output/thresholds.txt
    echo "✓ Thresholds loaded"
else
    echo "⚠ Thresholds file not found: $INPUT_THRESHOLDS"
    echo "  Will use thresholds generated from current data"
fi
...
echo ""
echo "Key outputs:"
if [ -f "$OUTPUT_DIR/ncopy/copy_numbers.table.txt" ]; then
    echo "✓ copy_numbers.table.txt: $OUTPUT_DIR/ncopy/copy_numbers.table.txt"
fi
if [ -f "$OUTPUT_DIR/ncopy/presence.table.txt" ]; then
    echo "✓ presence.table.txt: $OUTPUT_DIR/ncopy/presence.table.txt"
fi
...
echo ""
echo "Status: $ANCESTRY/$BATCH_ID complete"
