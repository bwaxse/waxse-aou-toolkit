...
def parse_loci(loci_list):
    ...
    for locus in loci_list:
        try:
            ...
            # Extract position - handle both chr:pos and chr:pos_ALLELE formats
            if '-' in pos_alleles:
                pos = int(pos_alleles.split('-')[0])
            else:
                pos = int(pos_alleles)
            ...
            print(f"Warning: Could not parse locus '{locus}': {e}")
            continue
...
gwas_catalog_files = {
    'condition__hpv': [
        'hpv_abnl_pap - EFO_0009511_associations_export.tsv', 
        'hpv_anal_carcinoma - MONDO_0003199_associations_export.tsv', 
        'hpv_cervical_cancer - MONDO_0002974_associations_export.tsv', 
        'hpv_cervical_carcinoma - EFO_0001061_associations_export.tsv', 
        'hpv_chronic_infection - EFO_0010060_associations_export.tsv', 
        'hpv_cin_2_3 - EFO_1000166_associations_export.tsv', 
        'hpv_common_wart - EFO_0009662_associations_export.tsv', 
        'hpv_dysplasia_of_cervix - EFO_1000910_associations_export.tsv', 
        'hpv_infection - EFO_0001668_associations_export.tsv', 
        'hpv_uterine_cervix_carcinoma_in_situ - MONDO_0042487_associations_export.tsv', 
        'hpv_vulvar_carcinoma - EFO_0002921_associations_export.tsv'
    ]
}
...
for condition, files in gwas_catalog_files.items():
    ...
    for file in files:
        ...
        file_path = f"{my_bucket}/saige_gwas/{file}"
...
hpv_catalog_df = gwas_catalog_dfs['condition__hpv']


# In[ ]:


hpv_catalog_df.group_by('efoTraits').len()['efoTraits'].to_list()
...
hpv_traits = [
    'human papilloma virus infection,oropharynx cancer',
    'human papilloma virus infection,oral cavity cancer',
    'cervical cancer',
    'HPV seropositivity,common wart',
    'cervical carcinoma',
    'dysplasia of cervix,cervical cancer',
    'dysplasia of cervix',
    'Cervical Intraepithelial Neoplasia Grade 2/3',
    'common wart',
    'human papilloma virus infection',
#    'cervical carcinoma,prostate carcinoma,biliary tract cancer,pancreatic carcinoma,ovarian cancer,lung cancer,colorectal cancer,breast carcinoma,hepatocellular carcinoma,non-Hodgkins lymphoma,esophageal cancer,endometrial cancer,gastric cancer',
    'chronic human papillomavirus infection',
    'cervical carcinoma,Cervical Intraepithelial Neoplasia Grade 2/3'
]
...
catalog_data = {
    'condition__hpv': (hpv_catalog_df, hpv_traits),
}
...
known_loci = {}
for condition in ['condition__hpv']:
...
def process_gwas(base_dir, ancestry, trait, known_loci_list, plot_params):
    ...
    if ancestry == 'metal':
        ...
        sumstats = gl.Sumstats(
            f'{base_dir}/{ancestry}/condition__hpv/gwas_results.tsv.gz',
            fmt="saige",
            snpid='vid',
            rsid=None,
            chrom='chromosome',
    ...
    highlighted_kwargs.update({
        ...
        'save': f'manhattan_plots/{trait}_{ancestry}_highlighted_mqq.png'
    })
    sumstats.plot_mqq(**highlighted_kwargs)
    
    # Plot regular MQQ
    regular_kwargs = base_plot_kwargs.copy()
    regular_kwargs['save'] = f'manhattan_plots/{trait}_{ancestry}_mqq.png'
...
base_dir = f'{my_bucket}/saige_gwas/v1_redo' # no trailing /


# In[ ]:


traits = ['condition__hpv']

# Define trait-specific plotting parameters
trait_plot_params = {
    'condition__hpv': {'cut': None, 'cutfactor': None, 'ylabels': [5,10,15,20,25]}
}
...
for trait in traits:
    ...
    if trait_sumstats:
        try:
            ...
            regular_kwargs['save'] = f'manhattan_plots/{trait}_stacked_m.png'
            ...
            highlighted_kwargs.update({
                ...
                'highlight_color': ['#DC143C', '#FFA500'],
                'save': f'manhattan_plots/{trait}_highlighted_stacked_m.png'
            })
            ...
            create_stacked_regional_plots(
                results, trait, ['eur', 'afr', 'amr', 'metal'], trait_sumstats, plot_titles, ref_populations
            )
...
# args = ["gsutil", "cp", "-r", "./manhattan_plots/", f"{my_bucket}/saige_gwas/v1_redo/"]
...
# # move all plots from manhattan_plots to bucket metal folder
# args = ["gsutil", "cp", "-r", "./regional_plots/", f"{my_bucket}/saige_gwas/v1_redo/"]
# output = subprocess.run(args, capture_output=True)
